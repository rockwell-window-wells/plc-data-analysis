...
class OperatorStatsPDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(60, 1, 'Operator Stats', 0, 0, 'R')
        self.ln(20)
    ...
    def page_body(self, datestart, dateend, plot, opnum, opname, ncycles, leadavg, assistavg, overavg, allavg):
        datetext = "Evaluation Period:\t{} to {}".format(str(datestart), str(dateend))
        ncyclestext = "Cycles Logged:\t{}".format(ncycles)
        ...
        self.cell(40, 6, ncyclestext, 0, 1, 'L')
        self.cell(40, 3, "", 0, 1, 'L')
        self.cell(40, 6, leadavgtext, 0, 1, 'L')
        self.cell(40, 6, assistavgtext, 0, 1, 'L')
        self.cell(40, 6, overavgtext, 0, 1, 'L')
        self.cell(40, 6, allavgtext, 0, 1, 'L')


    def print_page(self, datestart, dateend, plot, opnum, opname, ncycles, leadavg, assistavg, overavg, allavg):
        # Generates the report
        self.add_page()
        self.page_body(datestart, dateend, plot, opnum, opname, ncycles, leadavg, assistavg, overavg, allavg)

def generate_operator_PDF(datestart, dateend, plot, opnum, opname, ncycles, leadavg, assistavg, overavg, allavg, filename, exportpath):
    pdf = OperatorStatsPDF()
    pdf.print_page(datestart, dateend, plot, opnum, opname, ncycles, leadavg, assistavg, overavg, allavg)
...
def clean_single_mold_data(single_mold_data):
    ...
    df_layup = align_operator_times(df_cleaned, datetimes, "Layup Time", layup_inds, layup_times, lead_inds, assistant1_inds, assistant2_inds, assistant3_inds)
    df_close = align_operator_times(df_cleaned, datetimes, "Close Time", close_inds, close_times, lead_inds, assistant1_inds, assistant2_inds, assistant3_inds)
    df_resin = align_operator_times(df_cleaned, datetimes, "Resin Time", resin_inds, resin_times, lead_inds, assistant1_inds, assistant2_inds, assistant3_inds)
    df_cycle = align_operator_times(df_cleaned, datetimes, "Cycle Time", cycle_inds, cycle_times, lead_inds, assistant1_inds, assistant2_inds, assistant3_inds)
...
def get_operator_stats(df, timestring):
    ...
    for operator in unique_operators:
        ...
        if timestring == "Cycle Time":
            ncycles = operator_compare[operator_col].count()
            ...
            filename = "Operator_{}_{}_Stats.pdf".format(operator, timestring.replace(" ","_"))
            exportpath = os.getcwd()
            opname = lookup_operator_name(operator, "ID_data.xlsx")
            generate_operator_PDF(startdate, enddate, plotname, operator, opname, ncycles, leadavg, assistavg, overavg, allavg, filename, exportpath)
