...
def clean_single_mold_data(single_mold_data):
    ...
    if not all(x == indlens[0] for x in indlens):
        ...
        align_inds_times(layup_inds, layup_times, close_inds, longest_inds, longest_len)
        align_inds_times(close_inds, close_times, resin_inds, longest_inds, longest_len)
...
def get_operator_stats(df_collapse):
    ...
    for operator in unique_operators:
        ...
        operator_compare = pd.concat([operator_compare, df_collapse["Cycle Time"].rename("Team")], axis=1)

        operator_compare.boxplot(column = list(operator_compare.columns), grid=False)
        axes = plt.gca()
        axes.yaxis.grid()
