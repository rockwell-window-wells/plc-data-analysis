# -*- coding: utf-8 -*-
"""
Created on Thu Mar 26 07:36:17 2026
...
def generate_date_chunks(start_date, end_date, chunk_days):
    ...
    while current_start <= end_date:
        current_end = current_start + dt.timedelta(days=chunk_days - 1)
...
def sync_mold(datestart, dateend, mold_name, conn, ):
    """
    ...
    try:
        ...
        dtstart = dt.datetime.combine(datestart, dt.time(0, 0, 0))
        dtend = dt.datetime.combine(dateend, dt.time(23, 59, 59))
...
def run_backfill(datestart, dateend, chunk_days=CHUNK_DAYS, db_path=DB_PATH):
    ...
    # NEW: loop over date chunks
    for chunk_start, chunk_end in generate_date_chunks(datestart, dateend, chunk_days):
        print(f"\nProcessing chunk: {chunk_start} → {chunk_end}")

        for mold_name in api.molds:
            try:
                inserted, skipped = sync_mold(chunk_start, chunk_end, mold_name, conn)
                total_inserted += inserted
                total_skipped += skipped
            except Exception as e:
                errors.append((mold_name, f"{chunk_start} to {chunk_end}: {str(e)}"))
    ...
    if errors:
        print(f"Errors on {len(errors)} job(s):")
...
if __name__ == "__main__":
    datestart = dt.date(2026, 1, 1)
    dateend = dt.date(2026, 3, 20)
    run_backfill(datestart, dateend)
